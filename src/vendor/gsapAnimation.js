        ...
        if ($(".split-text").length > 0) {
            ...
            st.each(function (index, el) {
                ...
                const simpleSplit = (target) => {
                    ...
                    let chars = makeCharSpans();
                    let words = [];
                    let lines = [];
                    return {
                        get chars() { return chars; },
                        get words() { return words.length ? words : (words = makeWordSpans()); },
                        get lines() { return lines.length ? lines : (lines = this.words); },
                        split(opts) {
                            if (!opts || opts.type === 'words') {
                                words = makeWordSpans();
                            } else if (opts.type === 'lines') {
                                lines = makeWordSpans();
                            } else {
                                chars = makeCharSpans();
                            }
                ...
                const hasHighlight = $target.querySelector && $target.querySelector('span.active, span.type-2');
                const pxl_split = hasHighlight ? null : simpleSplit($target);
                let split_type_set = hasHighlight ? [$target] : pxl_split.chars;
                ...
                if (!hasHighlight && hasClass("split-words-scale")) {
